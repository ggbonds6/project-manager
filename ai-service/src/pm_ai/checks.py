...
def check_amount_case(pages: list[str]) -> dict:
    ...
    for page_no, text in enumerate(pages, 1):
        ...
        for seg in re.split(r"[。；;\n]", text):
            ...
            if match:
                items.append({"page": page_no, "status": "pass",
                              "detail": f"{match} ↔ {cn[0]}（一致）"})
            else:
                items.append({"page": page_no, "status": "fail",
                              "detail": f"大写「{cn[0]}」= {cn_num:,}，"
                                        f"但阿拉伯数字写作 {[a for _, a in ars]} —— **不一致，必须人工核对**"})
    if not checked:
        return {"name": "金额大小写互校", "status": "skip",
                "detail": "文中未找到可配对的大小写金额", "items": []}
    fails = [i for i in items if i["status"] == "fail"]
    return {
        "name": "金额大小写互校",
        "status": "fail" if fails else "pass",
        "detail": f"共核对 {checked} 处" + (f"，**{len(fails)} 处不一致**" if fails else "，全部一致"),
        "items": items,
    }
...
def check_percent_sum(pages: list[str]) -> dict:
    ...
    if not items:
        return {"name": "同段比例合计", "status": "skip", "detail": "未找到同段多比例的情形", "items": []}
    return {
        "name": "同段比例合计",
        "status": "warn" if any(i["status"] == "warn" for i in items) else "pass",
        "detail": f"检查 {len(items)} 段", "items": items,
    }
...
def check_number_style(pages: list[str]) -> dict:
    ...
    for text in pages:
        for raw in re.findall(r"[0-9０-９][0-9０-９,，.．]*", text):
            if len(raw) < 4:
                continue
            num = _to_decimal(raw.replace("０", "0").replace("１", "1").replace("２", "2")
    ...
    if not mixed:
        return {"name": "数值写法一致性", "status": "pass", "detail": "未发现同一数值的多种写法", "items": []}
    items = [{"status": "warn", "detail": f"{k} 出现了 {len(v)} 种写法：{' / '.join(v)}"}
             for k, v in list(mixed.items())[:12]]
    return {"name": "数值写法一致性", "status": "warn",
            "detail": f"{len(mixed)} 个数值存在多种写法（可能是全角/千分位混用）", "items": items}
...
_ANY_NUMBER_RE = re.compile(r"[¥￥]?\s*(\d{1,3}(?:[,，]\d{3})+(?:[.．]\d{1,2})?|\d+(?:[.．]\d{1,2})?)")
...
def verify_numbers_in_source(answer: str, pages: list[str]) -> dict:
    ...
    items = [{"status": "warn", "detail": f"{n:,} 未在原文中找到（可能是推断/计算，也可能是编造）"}
             for n in missing[:20]]
    return {
        "name": "答案数字可溯源",
        "status": "pass" if not missing else "warn",
        "detail": f"答案含 {len(ans)} 个数值，其中 {len(ans) - len(missing)} 个可在原文逐字找到"
