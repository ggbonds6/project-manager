...
function UsersTab() {
  ...
  const editUser = (row: AdminUser) => {
    open(
      ...
      { name: row.name, role: row.role, status: row.status },
      async (values) => {
  ...
  const columns: ColumnsType<AdminUser> = [
    ...
    { title: '最后登录', dataIndex: 'lastLoginTime', width: 180, render: fmtDateTime },
    {
      title: '操作',
      key: 'op',
      width: 220,
...
function DictsTab() {
  ...
  const columns: ColumnsType<DictRow> = [
    ...
    { title: '排序', dataIndex: 'sortNo', width: 90, align: 'right' },
    {
      title: '操作',
      key: 'op',
      width: 150,
...
function LogsTab() {
  ...
  const columns: ColumnsType<LogRow> = [
    ...
    { title: '时间', dataIndex: 'createTime', width: 180, render: fmtDateTime },
    { title: '用户', dataIndex: 'userName', width: 130, render: (v?: string | null) => v || '系统' },
    {
      title: '类型',
      dataIndex: 'bizType',
      width: 120,
