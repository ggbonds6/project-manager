...
> **先看两张地图**：[`docs/README.md`](docs/README.md)（**文档地图**：哪份是权威、哪份已核减）与
> [`scripts/README.md`](scripts/README.md)（**脚本与操作清单**：场景速查 + Shell 要求 + 全流程命令）。
>
...
├─ ai-backend/   AI 能力服务（Spring Boot 3 / Java 17，与主系统同栈、独立构建与部署 · ai-1.1.0；问答支持受控查询（项目/合同/付款/统计口径））
├─ frontend/     React 前端
...
1. 建库（自研迁移 Runner 在启动时自动执行 `db/migration-yashan/V1~V14` 建表与种子，幂等）：
...
- **崖山 YashanDB（Oracle 模式）主备集群**，驱动级 `primary` 多地址 + TAF 高可用（方言约定与踩坑见 [`docs/崖山数据库与迁移约定.md`](docs/崖山数据库与迁移约定.md)）。
- 迁移脚本：`backend/src/main/resources/db/migration-yashan/`（V1~V14），启动时由 `YashanMigrationRunner` 自动执行。
...
4. 涉及脚本时同步更新 `scripts/README.md`（脚本与操作清单；新增脚本必须登记，且**优先提供 Windows 原生入口**）；
5. **文档地图同步**：新增/合并/核减文档时必须更新 [`docs/README.md`](docs/README.md)（含"已核减文档"登记表），
   主系统与 AI 能力服务的文档改动要**双向登记**，确保两边不脱节；
6. **一个主题一份权威文档**：一次性/已完成的计划类文档在完成后**核减**——决策与实测数字并入 `ITERATION.md` 或权威文档，
   不要让过程性描述留在仓库里制造歧义、占用阅读成本；
7. **Markdown 表格两条禁忌**（曾把表格渲染打断）：表格中间**不能有空行**；单元格内的竖线必须写成 `\|`。
   改完文档**提交前跑一次** `node scripts/check-docs.mjs`（查表格断行 / 裸竖线 / 死链 / ITERATION 一致性，有问题退出码 1）；
8. **每次功能迭代都要用新增/修改演示数据把该功能体现出来**（含边界场景：缺件、异常、风险项），
   让系统可直接演示——这是固定交付项，不是可选项；
9. 收尾顺手清理无效代码（未使用导入 / 死方法 / 只写不读的变量）；
10. 提交说明引用对应验证结果。

当前版本：主系统 **v3.7.0**（含「AI 与知识库」P2 **受控查询**：项目/合同/付款/统计口径走主系统受控查询 RPC + 短时效 `scope_token`，口径与详情页同源；AI 侧条件注册 `query_business_data` 工具；前端新增「系统数据」区块；新增 `eval/smoke-30.json` 评测集 + `scripts/eval-ai.mjs`）｜ AI 能力服务 **ai-1.1.0**（Java 版，独立构建与部署；`/chat` 返回结构化引用；问答支持受控查询（项目/合同/付款/统计口径））｜ 文档与脚本 **docs-1.4**
