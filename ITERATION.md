...
| v2.1 | 2026-09-08 | 阶段说明/关键材料/画布布局（数据层） | Flyway V8：`phase_template.guide`(阶段说明)、`key_materials`(关键材料)；`project_phase` 快照同字段（创建项目时随模板带入，详情阶段 VO 透出）；`phase_tpl.flow_json`(draw.io 式节点坐标+连线)；模板保存/复制同步字段；模板 GET/{id} 与 PUT flow 接口 | `(待提交)` |
