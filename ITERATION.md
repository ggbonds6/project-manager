...
- **P0 规范化落地**（对应评估文档 §3.2 的 P0）：`ruff`（lint + format，规则集 `E/W/F/I/UP/B/SIM`，行宽 100）
  ...
  新增 **`[dev]` extra**（pytest + ruff）与 **`scripts/check.sh`**（**在 Docker 里跑 lint + 测试**，开发机无需装 Python）；
